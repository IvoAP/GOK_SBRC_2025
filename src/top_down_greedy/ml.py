...
def evaluate_models(datasets, pipelines, cv=2):
    ...
    os.makedirs('results/top_down_greedy', exist_ok=True)
    ...
    for dataset_idx, (dataset_name, dataset_info) in enumerate(datasets.items(), 1):
        ...
        for model_idx, (model_name, pipeline) in enumerate(pipelines.items(), 1):
            ...
            try:
                ...
                model_file = f'results/top_down_greedy/{model_name.replace(" ", "_")}_results.csv'
...
def main():
    data_dir = 'datasets/top_down_greedy'
    ...
    results_df.to_csv('results/top_down_greedy/all_results.csv', index=False)
    print("\nResults saved in results/top_down_greedy/")
