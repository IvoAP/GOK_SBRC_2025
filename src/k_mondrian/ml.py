...
def evaluate_models(datasets, pipelines, cv=2):
    ...
    os.makedirs('results/mondrian', exist_ok=True)
    ...
    for dataset_idx, (dataset_name, dataset_info) in enumerate(datasets.items(), 1):
        ...
        for model_idx, (model_name, pipeline) in enumerate(pipelines.items(), 1):
            ...
            try:
                ...
                model_file = f'results/mondrian/{model_name.replace(" ", "_")}_results.csv'
...
def main():
    ...
    results_df.to_csv('results/mondrian/all_results.csv', index=False)
    print("\nResults saved in results/mondrian/")
